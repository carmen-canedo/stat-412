...
long_deaths_global <- deaths_global %>% 
  pivot_longer(cols = ends_with("20"), names_to = "date", values_to = "deaths_count") %>% 
  group_by(`Country/Region`, date) %>% 
  summarise(daily_deaths_count = sum(deaths_count), population = sum(Population.y)) %>% 
  arrange(date) %>% 
  rename(country_region = `Country/Region`)
```
...
global <- global %>% 
  rename(date = date.x) %>% 
  select(country_region, population, date, daily_case_count, daily_deaths_count,-date.y)
```

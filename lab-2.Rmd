...
```{r, message=FALSE}
confrimed_global <- read_csv("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_confirmed_global.csv")
...
totals_cases_US <- confirmed_US %>% 
  pivot_longer(cols = ends_with("20"), names_to = "date", values_to = "cases_count") %>% 
  group_by(Province_State) %>% 
  mutate(total_cases = cumsum(cases_count))

# Lengthening US deaths
deaths_US <- deaths_US %>% 
  pivot_longer(cols = ends_with("20"), names_to = "date", values_to = "deaths_count") %>% 
  group_by(state) %>% 
  mutate(total_deaths = sum(deaths_count) / 1000000)


US_by_state <- confirmed_US %>% 
  left_join(deaths_US, by = "date")
```
